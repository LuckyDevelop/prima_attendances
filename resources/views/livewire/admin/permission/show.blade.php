<div>
    <x-admin.page-header title="Detail Izin #{{ $perm->id }}">
        <x-slot:actions>
    ...
    <div class="mx-auto max-w-3xl space-y-6">
        ...
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
            ...
            <div class="flex items-center gap-4">
                <div class="h-14 w-14 flex-shrink-0 overflow-hidden rounded-full bg-gray-200">
                    @if($perm->user?->photo)
                        <img src="{{ Storage::url($perm->user->photo) }}" class="h-full w-full object-cover" alt="" />
                    @else
                        <span class="flex h-full w-full items-center justify-center text-xl font-semibold text-gray-500">
                            {{ strtoupper(substr($perm->user?->full_name ?? '?', 0, 1)) }}
                        </span>
                    @endif
                </div>
                <div>
                    <p class="font-semibold text-gray-900">{{ $perm->user?->full_name ?? '—' }}</p>
                    <p class="text-sm text-gray-500">{{ $perm->user?->employee_id }} · {{ $perm->user?->department?->name ?? '—' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
            ...
            <dl class="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                    ...
                    <dd class="font-medium text-gray-900">{{ \Carbon\Carbon::parse($perm->request_date)->isoFormat('D MMMM YYYY') }}</dd>
                </div>
                @if($perm->start_time && $perm->end_time)
                <div>
                    <dt class="text-gray-500">Waktu</dt>
                    <dd class="font-medium text-gray-900">{{ substr($perm->start_time, 0, 5) }} – {{ substr($perm->end_time, 0, 5) }}</dd>
                </div>
                @endif
                <div>
                ...
                <div class="col-span-2">
                    ...
                    <dd class="mt-1 text-gray-700">{{ $perm->reason }}</dd>
                </div>
                @endif
            </dl>
            @if($perm->attachment)
                <div class="mt-4 border-t border-gray-100 pt-4">
                    <a href="{{ Storage::url($perm->attachment) }}" target="_blank"
        ...
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
            ...
            <ol class="relative border-l border-gray-200 pl-6 space-y-4">
                @foreach($perm->approvals->sortBy('level') as $approval)
                <li class="ml-2">
                    @php
    ...
    {{-- Modal --}}
    <x-ui.modal name="perm-action" max-width="md">
        <x-slot:title>
            {{ $pendingAction === 'approve' ? 'Setujui Izin' : 'Tolak Izin' }}
        </x-slot:title>
        ...
        <x-slot:footer>
            <button @click="$dispatch('close-modal', { name: 'perm-action' })"
