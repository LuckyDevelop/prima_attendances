<div>
    <x-admin.page-header title="Laporan Absensi">
        <x-slot:actions>
    ...
    <div class="hidden print:block mb-6">
        <h1 class="text-xl font-bold text-gray-900">Laporan Absensi Karyawan</h1>
    ...
    @if ($rows->isNotEmpty())
        <div class="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4 print:grid-cols-4">
            ...
            <div class="rounded-xl bg-white p-4 shadow-sm ring-1 ring-gray-200 text-center">
                <div class="text-2xl font-bold text-green-600">{{ $rows->sum('present_count') }}</div>
                <div class="text-xs text-gray-500 mt-1">Total Hadir</div>
            </div>
            <div class="rounded-xl bg-white p-4 shadow-sm ring-1 ring-gray-200 text-center">
                <div class="text-2xl font-bold text-yellow-600">{{ $rows->sum('late_count') }}</div>
                <div class="text-xs text-gray-500 mt-1">Total Terlambat</div>
            </div>
            <div class="rounded-xl bg-white p-4 shadow-sm ring-1 ring-gray-200 text-center">
                <div class="text-2xl font-bold text-red-600">{{ $rows->sum('absent_count') }}</div>
                <div class="text-xs text-gray-500 mt-1">Total Tidak Hadir</div>
            </div>
        </div>
    @endif
                    ...
                    @forelse ($rows as $row)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-center">
                                <span class="font-semibold text-green-700">{{ $row->present_count }}</span>
                            </td>
                            <td class="px-4 py-3 text-center">
                                @if ($row->late_count > 0)
                                    <span class="font-semibold text-yellow-700">{{ $row->late_count }}</span>
                                @else
                            ...
                            <td class="px-4 py-3 text-center">
                                @if ($row->absent_count > 0)
                                    <span class="font-semibold text-red-700">{{ $row->absent_count }}</span>
                                @else
                                    <span class="text-gray-400">0</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="text-blue-700">{{ $row->leave_count }}</span>
                            </td>
                            <td class="px-4 py-3 text-center text-gray-700">{{ $row->total_count }}</td>
                            <td class="px-4 py-3 text-right text-gray-700">
                                @php
                                    $hours = intdiv((int) $row->total_work_min, 60);
                                    $mins  = (int) $row->total_work_min % 60;
                                @endphp
                                {{ $hours }}j {{ $mins }}m
                            </td>
                        </tr>
                ...
                @if ($rows->isNotEmpty())
                    <tfoot>
                        <tr class="border-t border-gray-200 bg-gray-50 font-semibold text-gray-700">
                            ...
                            <td class="px-4 py-3 text-center text-green-700">{{ $rows->sum('present_count') }}</td>
                            <td class="px-4 py-3 text-center text-yellow-700">{{ $rows->sum('late_count') }}</td>
                            <td class="px-4 py-3 text-center text-red-700">{{ $rows->sum('absent_count') }}</td>
                            <td class="px-4 py-3 text-center text-blue-700">{{ $rows->sum('leave_count') }}</td>
                            <td class="px-4 py-3 text-center">{{ $rows->sum('total_count') }}</td>
                            <td class="px-4 py-3 text-right">
                                @php
                                    $totalMins = $rows->sum('total_work_min');
                                    $th = intdiv((int) $totalMins, 60);
                                    $tm = (int) $totalMins % 60;
                                @endphp
                                {{ $th }}j {{ $tm }}m
                            </td>
